...
define( 'NOTIFIER_THEME_NAME', G7_NAME ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', G7_FOLDERNAME ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://gawibowo.com/themes/' . G7_FOLDERNAME . '/notifier.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		$theme_data = wp_get_theme(G7_FOLDERNAME);
		$theme_version = $theme_data->Version;

		if( (float)$xml->latest > (float)$theme_version) { // Compare current theme version with the remote XML version
			add_theme_page( NOTIFIER_THEME_NAME . ' Theme Updates', NOTIFIER_THEME_NAME . ' <span class="update-plugins count-1"><span class="update-count">New</span></span>', 'administrator', 'theme-update-notifier', 'update_notifier');
		}
	}
}
...
function update_notifier() {
	...
	$theme_data = wp_get_theme(G7_FOLDERNAME);
	$theme_version = $theme_data->Version;
	$themeforest_url = 'http://www.themeforest.net/';
	?>
	...
	<div class="wrap">
		...
		<div id="instructions">
		    ...
		    <p>To update the Theme, login to <a href="<?php echo $themeforest_url; ?>">ThemeForest</a>, head over to your <strong>downloads</strong> section and re-download the theme like you did when you bought it.</p>
